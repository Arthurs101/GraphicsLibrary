...
void Render3DObjects(char PRIMTYPE = 't'){
    for (int i = 0 ; i < objects.size() ; i++){
        ...
        for (int face = 0 ; face < objects[i].faces.size() ; i++ ){
            vector<float> v0 = objects[i].faces[face][0];
            vector<float> v1 = objects[i].faces[face][1];
            vector<float> v2 = objects[i].faces[face][2];
            if (objects[i].faces[face].size() == 4 ) {
                vector<float> v3 = objects[i].faces[face][3];
